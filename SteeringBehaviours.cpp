...

{
	...
	m_vWanderTarget = Vector2(m_dWanderRadius * cos(theta),
...
//---------------------------------dtor ----------------------------------
SteeringBehaviour::~SteeringBehaviour() { delete m_pPath; }
...
//  in m_SummingMethod
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Calculate()
{
	...
	default:m_vSteeringForce = Vector2(0, 0);
...
//  force to add.
//------------------------------------------------------------------------
bool SteeringBehaviour::AccumulateForce(Vector2 &RunningTot,
	Vector2 ForceToAdd)
{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::CalculatePrioritized()
{
	Vector2 force;
...
//  returning
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::CalculateWeightedSum()
{
	if (On(wall_avoidance))
	{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::CalculateDithered()
{
	//reset the steering force
	m_vSteeringForce.Zero();
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Seek(Vector2 TargetPos)
{
	Vector2 DesiredVelocity = Vec2DNormalize(TargetPos - m_pVehicle->Pos())
		* m_pVehicle->MaxSpeed();
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Flee(Vector2 TargetPos)
{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Arrive(Vector2     TargetPos,
...
//  evader
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Pursuit(const Vehicle* evader)
{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Evade(const Vehicle* pursuer)
{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Wander()
{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::ObstacleAvoidance(const std::vector<GameObject*>& obstacles)
{
	...
	std::vector<GameObject*>::const_iterator curOb = obstacles.begin();
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::WallAvoidance(const std::vector<Wall2D>& walls)
{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Separation(const vector<Vehicle*> &neighbors)
{
	Vector2 SteeringForce;
...
//  of its neighbors
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Alignment(const vector<Vehicle*>& neighbors)
{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Cohesion(const vector<Vehicle*> &neighbors)
{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::SeparationPlus(const vector<Vehicle*> &neighbors)
{
	Vector2 SteeringForce;

	//iterate through the neighbors and sum up all the position vectors
	for (GameObject* pV = m_pVehicle->World()->CellSpace()->begin();
...
//  USES SPACIAL PARTITIONING
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::AlignmentPlus(const vector<Vehicle*> &neighbors)
{
...
//  USES SPACIAL PARTITIONING
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::CohesionPlus(const vector<Vehicle*> &neighbors)
{
	//first find the center of mass of all the agents
	Vector2 CenterOfMass, SteeringForce;
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Interpose(const Vehicle* AgentA,
...
//
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::Hide(const Vehicle*           hunter,
	const vector<GameObject*>& obstacles)
{
	...
	std::vector<GameObject*>::const_iterator curOb = obstacles.begin();
	std::vector<GameObject*>::const_iterator closest;
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::GetHidingPosition(const Vector2& posOb,
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::FollowPath()
{
...
//------------------------------------------------------------------------
Vector2 SteeringBehaviour::OffsetPursuit(const Vehicle*  leader,
...
//
//------------------------------------------------------------------------
void SteeringBehaviour::RenderAids()
{
	...
	if (On(wander) && m_pVehicle->World()->RenderWanderCircle())
	{
		...
		Vector2 m_vTCC = PointToWorldSpace(Vector2(m_dWanderDistance*m_pVehicle->BRadius(), 0),
			m_pVehicle->Heading(),
			m_pVehicle->Side(),
			m_pVehicle->Pos());
		...
		gdi->RedPen();
		gdi->Circle(PointToWorldSpace((m_vWanderTarget + Vector2(m_dWanderDistance, 0))*m_pVehicle->BRadius(),
			m_pVehicle->Heading(),
			m_pVehicle->Side(),
			m_pVehicle->Pos()), 3);
	}
	...
	if (m_pVehicle->World()->RenderDetectionBox())
	{
		...
		box[0] = Vector2(0, m_pVehicle->BRadius());
		box[1] = Vector2(length, m_pVehicle->BRadius());
		box[2] = Vector2(length, -m_pVehicle->BRadius());
		box[3] = Vector2(0, -m_pVehicle->BRadius());
		...
		std::vector<GameObject*>::const_iterator curOb = m_pVehicle->World()->Obstacles().begin();
