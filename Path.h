...
public:
	...
	inline void AddWayPoint(Vector2 new_point) { m_WayPoints.push_back(new_point); };

	//methods for setting the path with either another Path or a list of vectors
	void Set(std::list<Vector2> new_path) { m_WayPoints = new_path; curWaypoint = m_WayPoints.begin(); }
	void Set(const Path& path) { m_WayPoints = path.GetPath(); curWaypoint = m_WayPoints.begin(); }


	void Clear() { m_WayPoints.clear(); }

	std::list<Vector2> GetPath()const { return m_WayPoints; }
